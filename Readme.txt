...
Instructions:
The models implemented here take CSF proteomic data from the SomaScan 7K Assay by SomaLogic and predict the chronological age of individuals. There are two main models described in the paper. A "Full Model" which 1,157 protein measurements to make the prediction and a "Minimal Model" which uses just 109 proteins. Please refer to the paper and the instructions below before attempting to run any models. It is very important the input data be structured properly for each model or the code will not run properly. For this reason, template files have been provided with the order and names of the SomaLogic features to be used. Please refer to the Extended Materials excel file from the paper for detailed information about each protein feature (including full protein names and gene ids).
